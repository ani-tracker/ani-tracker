# 内置 libtorrent 下载引擎实施计划（归档快照）

> 归档日期：2026-07-24。计划实现已完成；本文保留当时阶段状态和外部验收记录，不作为当前待办依据。

最近更新：2026-07-23
状态：桌面阶段已实现，跨平台发布产物与移动宿主继续实施。

## 当前实施状态

| 范围 | 状态 |
| --- | --- |
| C++ `torrent-core`、NDJSON 协议、session/fastresume 恢复 | 已实现并通过真实进程握手 |
| Electron 进程托管、超时、日志、优雅停止 | 已实现 |
| 磁链/种子添加、任务刷新、暂停、恢复、移除、文件优先级 | 已接入 libtorrent |
...
- 设置 -> 下载核心 -> 使用内置核心的 UI：缩小另外两个选项，方便并列放下三个；内置核心 WebUI 地址和用户名密码不可编辑。
## 平台范围
