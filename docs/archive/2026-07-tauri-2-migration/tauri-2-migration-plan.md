...
- Android/iOS 已接通受 HTTP/HTTPS、有效主机和无内嵌凭据约束的系统外链能力；Renderer 不能调用任意 Intent、任意 URL Scheme 或本地文件协议。
...
- Windows、macOS 与 Android 发布任务要求项目长期自签凭据并校验证书；macOS 输出 x64/arm64 自签 DMG，iOS 将未签名 ARM64 设备 App 封装为供用户自行重签的 IPA，不依赖应用市场证书。
...
- Electron 主进程、preload、客户端、配置与 Node 原生依赖，以及 Capacitor Android/iOS 宿主和桥接已移入 `archive/legacy-hosts`；回退标签固定为 `legacy-hosts-final`，提交为 `6caf060f7247576f0f2f49d6ba9892e1149ed236`。
...
- 根 `package.json` 已移除退役桌面的兼容别名，默认开发、构建和打包命令均由防回流门禁锁定为 Tauri；ArtPlayer 与 HLS.js 仅供桌面远程 PWA 使用，继续保留。
...
- 2026-07-27 在 macOS 完成 P8 本地原生产物复核：生成 x64 DMG、Android ARM64 Debug APK 与 iOS ARM64 未签名 IPA；三组产物清单、SHA-256、Renderer/移动包边界和 Android Kotlin 策略测试均通过。
- Android Debug/Release 本地脚本均固定 `--target aarch64`，避免误编译 armv7、i686 和 x86_64；对应回归测试已纳入移动门禁。
- 本机 macOS DMG 为未签名构建验证件；Windows/Linux 构建、Windows/macOS/Android 正式自签、升级安装和全平台真机媒体矩阵仍按发布工作流及人工验收执行，不以本次本地结果代替。
