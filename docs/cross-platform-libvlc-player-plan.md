...
| P1 UI | PC、移动竖屏、移动横屏和中央错误提示；控制层自动隐藏 | 四个目标视口无溢出/遮挡；核心布局与 Stitch 接近；键盘和触屏可操作 | 进行中 |
| P2 Electron | libVLC 生命周期服务、双窗口协调、无边框/全屏、轨道与进度事件 | Windows/macOS/Linux 构建成功；窗口同步；关闭后无 VLC/窗口残留 | 待开始 |
| P3 远程网页 | 新 UI 接入 ArtPlayer 适配器，保留 HLS/字幕/外部播放器逻辑 | 现有远程 URL 与 API 不变；HLS、字幕、进度和错误恢复回归通过 | 待开始 |
